...
6. RAG 검색/응답 품질 변경이면 `RAG_EVAL_REPORT_PATH`로 지정한 파일 또는 `bash scripts/check_rag_eval.sh` 출력의 `rag_eval_report=<path>` 리포트를 확인해 summary를 요약합니다. 기본 RAG 리포트 위치는 `tests/reports/chatbot/rag_eval_report.json`입니다. 리포트가 없으면 평가 실행 가능 여부를 판단하고, 실행하지 못한 이유를 남은 리스크에 남깁니다.
...
RAG 관련 변경을 리뷰할 때 `RAG_EVAL_REPORT_PATH`로 지정한 파일 또는 평가 명령 출력의 `rag_eval_report=<path>` 리포트가 있으면 다음 지표를 짧게 요약합니다. `scripts/check_rag_eval.sh`의 기본 RAG 리포트 위치는 `tests/reports/chatbot/rag_eval_report.json`입니다.

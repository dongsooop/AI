...
이번 브랜치는 기준선 수집기, 서비스별 측정 도구, 제한된 부하 실행기, 공개 안전
요약기와 회귀 검사를 제공하는 데까지를 완료 범위로 봅니다. 실제 OCI 부하 실행과
운영 수치 확정은 배포 환경에서 필요할 때 별도로 수행하며, 이번 변경의 병합 조건으로
두지 않습니다.

`run_ai_service_load.py`는 자동 실행되거나 운영 트래픽을 발생시키지 않습니다.
운영자가 명시적으로 실행할 때만 동작하는 선택적 도구입니다.

...
| fallback cell rate | `ocr_fallback_cell_count`, `ocr_task_cell_count` | `sum(ocr_fallback_cell_count) / sum(ocr_task_cell_count)` |
| fallback cell count | `ocr_fallback_cell_count` | workload 참고값이며 단독 degraded 판정에서 제외 |
...
`OCR task workload`와 fallback cell count는 요청량과 이미지 복잡도를 설명하는 참고 신호입니다. fallback rate, queue pressure, latency를 요청별 task 수로 분해하거나 정규화할 때 사용합니다. `ocr_task_cell_count=0`인 event는 fallback cell rate의 분모에서 제외합니다.
...
| fallback direct answer route rate | `chatbot_request_summary` | `direct_answer_route=true`이고 `fallback=true`인 비율을 `mode`별 집계 |
| cache hit rate | `chatbot_request_summary.cache_hit` | `mode`별 `true` 비율을 latency/fallback과 함께 보는 보조 신호 |

BM25 tier가 `pickle`이 아니면 degraded 후보로 봅니다. 특히 `runtime_tokenize`는 startup 또는 retrieval 성능 리스크가 커질 수 있으므로 별도 집계합니다.

cache hit rate는 cold cache나 요청 유형에 따라 낮아질 수 있으므로 단독 degraded 판정에 사용하지 않습니다.

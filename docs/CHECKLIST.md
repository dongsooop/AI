...
RAG 검색/응답 품질에 영향을 주는 변경이라면 추가로 다음을 확인합니다.

- 변경이 retrieval, reranking, metadata boost, schedule routing, answer formatting 중 어디에 영향을 주는지 분류합니다.
- 검색용 텍스트, 답변용 텍스트, BM25 토큰, dense embedding 입력이 의도와 다르게 섞이지 않았는지 확인합니다.
- `query_index.py`의 새 스키마 의존이 `query_index_schema.py`와 맞고, 구버전 `search_df.parquet` fallback을 깨뜨리지 않는지 확인합니다.
- 연락처, 학과, 정책, 기숙사, 졸업, 개인정보, 학사일정 케이스가 특정 카테고리만 좋아지고 다른 카테고리가 후퇴하지 않았는지 확인합니다.
...
RAG 검색 품질이나 retrieval/reranking/metadata 변경을 검토할 때는 가능하면 다음 경량 평가도 실행합니다.

```bash
bash scripts/check_rag_eval.sh
```

리뷰에는 평가 리포트의 `total`, `passed`, `failed`, `pass_rate`, `top1_url_accuracy`, `top3_url_accuracy`, `answer_keyword_pass_rate`, `source_url_pass_rate`, `date_pass_rate`, `hallucination_proxy_rate`, `avg_retrieval_latency_ms`, `p95_retrieval_latency_ms`를 짧게 요약합니다. 카테고리별 `pass_rate`가 낮거나 이전보다 후퇴한 항목은 기능 또는 회귀 리스크로 분류합니다.
